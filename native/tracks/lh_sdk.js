...
function newAppLife(target) {
  if (target.onLaunch && typeof target.onLaunch === 'function') {
    initAppExtension(target)
  }
  return target
}
function initAppExtension(app) {
  let that = this
  // 返回一个新的 App 构造器
  const {
    onLaunch,
...
function initPageExtension(page) {
  ...
  page.onShareAppMessage = function (options) {
    let store_device_id = wx.getStorageSync('$ts_device_id')
    let current_path = utils.getActivePage().route
    console.log('_originParmaStr', this._originParmaStr)
    const oldShareInfo = onShareAppMessage.call(this, options) || {
      path: current_path + this._originParmaStr
    }
    console.log('oldshareinfo:', oldShareInfo.path)
    const shareInfo = this.$yh_share
    let times = Number(shareInfo.times) || 0
    console.log(' shareInfo:', shareInfo)
    if (store_device_id === shareInfo.from) shareInfo.times = times
    else {
      times += 1
      shareInfo.from = store_device_id
    }
    shareInfo.path = current_path
    shareInfo.times = times
    shareInfo.stamp = Date.now()
    oldShareInfo.path = oldShareInfo.path || ''
    oldShareInfo.path = shareInfo.path + '&yh_share=' + JSON.stringify(shareInfo)
    ...
    service.event({
      ...
      event_param: {
        share_time: times,
        share_page: current_path
      }
    })
    console.log('分享之后生成的路由', oldShareInfo.path)
    return oldShareInfo
  }
  /**
   * @method 重写 onLoad 方法
  ...
  page.onLoad = function (options) {
    ...
    for (let key in options) {
      if (key === 'yh_share') continue
      originParmaArr.push(key + '=' + options[key])
    }
    console.log('除了yh_share的路由originParmaArr:', originParmaArr)
    let originParmaStr = '?' + originParmaArr.join('&')
    this._originParmaStr = originParmaStr
    console.log('this.originparamstr:', this._originParmaStr)
    this.$yh_share = JSON.parse(decodeURIComponent(options.yh_share || '{}'))
    console.log('分享信息:', this.$yh_share)
    this.$yh_share_form = this.$yh_share.from
...
service.startSession = function () {
  ...
  //发送会话开始请求！！！
  console.log(requestParam, ' * 采集startSession数', requestParam.key)
...
Page = (page) => globalVarPage(newPageLife(page))
/**
 * 采集事件数据
 */
