...
_.getNet = function (callback) {
  wx.getNetworkType({
    complete: function (r) {
      _.getAuthorize({ network_type: r['networkType'] }, callback)
    }
  })
}
_.getAuthorize = function (data, callback) {
  wx.getSetting({
...
_.getPageUrl = function () {
  var pageUrl = pageAttribute.currentPath;
  if (pageAttribute.pageQueryString && pageAttribute.pageQueryString !== '') {
    pageUrl = pageAttribute.currentPath + '?' + pageAttribute.pageQueryString;
  }
  return pageUrl;
}
_.getUtm = function () {
...
_.sendRequest = function (data) {
  data.current_path = pageAttribute.currentPath; // 当前路径
  data.scene = scene // 场景
  data.page_id = data.page_id || pageAttribute.currentPath;
...
_.trackingPageView = function (data) {
  ...
  var _data = {
    ...
    start_session_time: sessionAttribue.startSessionTime,
    prev_session_id: sessionAttribue.prev_session_id
  }
...
ests.autoTrackCustom = {
  ...
  pageLoad: function (query) {
    ests.pageCustom = {}
    pageAttribute.currentPath = this.__route__
    pageAttribute.pageQueryString = _.getQueryString(query)
  },
  pageShow: function () {
    pageAttribute.currentPath = _.getCurrentPage().route
    ests._sessionStart()
    var data = {};
    data.prev_path = pageAttribute.prev_path;
...
ests._sessionStart = function () {
  ...
  data.prev_session_id = sessionAttribue.prev_session_id
  data.prev_path = pageAttribute.prev_path;
  sessionAttribue.startSessionTime = data.start_session_time;
  pageAttribute.prev_path = pageAttribute.currentPath;
  _.sendRequest(data)
}
ests._sessionEnd = function () {
  ...
  data.prev_session_id = sessionAttribue.prev_session_id;
  pageAttribute.prev_path = pageAttribute.currentPath;
  sessionAttribue.prev_session_id = this.store.getSessionId()
  ests.pageCustom.page_name = '' // 清空用户定义参数
  ests.pageCustom.pageview_param = ''
  data.session_duration = ((data.end_session_time - sessionAttribue.startSessionTime) / 1000).toFixed(2)
  _.sendRequest(data)
}
...
ests.event = function (eventParams) {
  if (eventParams.event_name && eventParams.event_id) {
    ...
    eventParams.start_session_time = sessionAttribue.startSessionTime;
    eventParams.prev_path = sessionAttribue.prev_session_id;
    eventParams.prev_path = pageAttribute.prev_path;
    eventParams.page_title = _.getPageTitle();
    eventParams.page_name = ests.pageCustom.page_name || _.getPageTitle();
    eventParams.session_duration = ((new Date().getTime() - sessionAttribue.startSessionTime) / 1000).toFixed(2)
...
ests.init = function (initData, config) {
  var appKey = initData.app_key
